...
export default function CreateSupportChat() {
    ...
    return (
        <div className={`w-full h-[89vh] bg-[url('/login/gradient_bg.png')] object-cover bg-cover bg-center bg-no-repeat ${MainFont.className} text-white `}>
            <Header />
            <div className='h-full mt-[100px] flex flex-col justify-center items-center gap-4 text-balance text-center'>
                ...
                <div className="flex justify-center items-center h-auto max-w-[730px] py-4 px-6 bg-[rgba(6,6,6,.64)] rounded-lg select-none">
                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center items-center gap-4">
                        ...
                        <motion.p
                            ...
                            animate={{ opacity: clientErrors.title?.message ? 1 : 0, height: clientErrors.title?.message ? 30 : 0}}
                        ...
                        <div className="w-full flex flex-col text-center">
                            <motion.p
                                initial={{ opacity: 0, height: 0 }}
                                animate={{ opacity: clientErrors.description?.message ? 1 : 0, height: clientErrors.description?.message ? 30 : 0}}
                                transition={{ duration: .3}}
                                className="text-orange-300 text-[13px] sm:text-[18px]"
                            >
                        ...
                        <div className={`w-full flex flex-col items-start ${ selectedFiles?.length === 0 || selectedFiles === null ? null : 'gap-2' }`}>
                            <motion.p
                                initial={{ opacity: 0, height: 0 }}
                                animate={{ opacity: clientErrors.file?.message ? 1 : 0, height: clientErrors.file?.message ? 30 : 0}}
                                transition={{ duration: .3}}
                                className="text-orange-300 text-[13px] sm:text-[18px] place-self-center"
                            >
                                {clientErrors.file?.message}
                            </motion.p>
                            <label htmlFor="file-input" className="place-self-start select-none">
                                <input id="file-input" type="file" {...register("file", {
                                    ...
                                    className="hidden"
                                />
                                <button type="button" className="py-2 px-4 bg-[#C2724F] rounded cursor-pointer select-none " onClick={() => document.getElementById('file-input')?.click()}>FILES</button>
                            </label>

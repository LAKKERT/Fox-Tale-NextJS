    ...
    if (req.method === 'POST') {
        try {
            ...
            try {
                ...
                return res.status(401).json({ message: 'Invalid token', redirectUrl: '/' });
            }
           ...
           try {
            ...
            if (userCode === null) {
                return res.status(404).json({ message: 'User not found', redirectUrl: '/' });
            }
            ...
            if (userCode === code) {
                ...
                return res.status(400).json({ message: 'Invalid verification code', redirectUrl: '/' });
            }
        ...
        return res.status(405).json({ message: 'Method not allowed', redirectUrl: '/' });
    }
}
